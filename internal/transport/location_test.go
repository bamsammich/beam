...
func TestParseLocationBeamURL(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			input:     "beam://mytoken@nas:9876/data",
			wantHost:  "nas",
			wantPort:  9876,
			wantPath:  "/data",
			wantToken: "mytoken",
...
func TestLocation_String(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			want: "beam://mytoken@nas:9876/backup",
		},
		{
			name: "beam without token",
			loc:  transport.Location{Scheme: "beam", Host: "nas", Port: 9876, Path: "/data"},
			want: "beam://nas:9876/data",
		},
		{
			name: "beam default port",
